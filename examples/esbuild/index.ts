...
const simpleLuminars = {
   ...
   minify: {
      type: Boolean,
      description: 'Minify the output (sets all --minify-* luminars)',
   },
   outdir: {
      type: String,
...
const app = cli({
   ...
   help: {
      ...
      render(nodes, renderers) {
         const [, usage, luminars, examples] = nodes

         // Replace "luminars" with "options"
         usage.data.body = usage.data.body.replace('luminars', 'options')
         ...
         const { tableData: luminarsTableData } = luminars.data.body.data
            ...
            // Split Luminars into "Simple options" and "Advanced options"
            {
               ...
               data: {
                  ...
                  body: {
                     ...
                     data: {
                        ...luminars,
                        tableData: luminarsTableData.filter(
                           ([luminarName]: [{ data: { name: string } }]) => luminarName.data.name in simpleLuminars,
                        ),
                     },
                  },
                  indentBody: 0,
               ...
               data: {
                  ...
                  body: {
                     ...
                     data: {
                        ...luminars,
                        tableData: luminarsTableData.filter(
                           ([luminarName]: [{ data: { name: string } }]) => luminarName.data.name in advancedLuminars,
                        ),
                     },
                  },
                  indentBody: 0,
