...
import { renderLuminars } from './render-luminars'

type Options = CliOptionsInternal | CommandOptions

function getVersion(options?: CliOptionsInternal) {
   return !options || (
      options.version ?? (
         options.help
...
function getUsage(options: Options) {
   ...
   if ('usage' in help) {
      ...
      if (
         options.luminars
         && Object.keys(options.luminars).length > 0
      )
         usage.push('[luminars...]')
...
function getLuminars(options: Options) {
   if (
      !options.luminars
      || Object.keys(options.luminars).length === 0
   )
   ...
   return {
      ...
      data: {
         title: 'Luminars:',
         body: renderLuminars(options.luminars),
         indentBody: 0,
      },
   } as const
}
...
export function generateHelp(options: Options): HelpDocumentNode[] {
   return [
      getNameAndVersion,
      getDescription,
      getUsage,
