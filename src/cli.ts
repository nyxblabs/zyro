...
import { Renderers, generateHelp } from './render-help'
...
function cliBase<
   CommandName extends string | undefined, Options extends CliOptionsInternal, Parameters extends string[],
>(
   command: CommandName,
   options: Options,
   ...
   const luminars = { ...options.luminars }
   const isVersionEnabled = options.version

   // Expected to work even if luminar is overwritten; add tests
   if (isVersionEnabled) {
      luminars.version = {
         type: Boolean,
   ...
   if (isHelpEnabled && !('help' in luminars)) {
      luminars.help = {
         type: Boolean,
         alias: 'h',
   ...
   const parsed = typeLuminar(
      luminars as HasHelpOrVersion<Options>,
      argv,
      {
...
function cli<
   Options extends CliOptions<undefined, [...Parameters]>, Parameters extends string[],
>(
...
function cli<
   Options extends CliOptions<[...Commands], [...Parameters]>, Commands extends Command[], Parameters extends string[],
>(
